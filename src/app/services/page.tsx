      ...
      <div className="container mx-auto px-4 max-w-6xl space-y-12">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          {services.map((service, idx) => (
            <Card key={service.id || idx} className="flex flex-col border border-slate-200 hover:border-[#6cb166]/50 hover:shadow-xl transition-all duration-300 rounded-2xl bg-white overflow-hidden p-8 space-y-4">
              ...
              <div className="pt-2">
                <Button asChild variant="outline" className="border-[#3b66b0] text-[#3b66b0] hover:bg-[#3b66b0] hover:text-white font-headline text-xs font-bold">
                  <Link href={service.link || '/contact'} className="flex items-center gap-2">
                    {service.cta || 'Learn More'} <ArrowRight className="w-3.5 h-3.5" />
                  </Link>
                </Button>
              </div>
            </Card>
